...
int div_func(int r1, int r2, int immS, int *mem, int *regs) {
    long long op1 = regs[r1] | ( ((long long) regs[r1+1]) << 32);
...
int emulate(map <string, int> cmdsMap, map<string, int> registersMap) {
    FILE *in = fopen("input.bin", "rb");
    int *mem = (int *)malloc(sizeof(int) * MEM_SIZE);
    int *regs = (int *)malloc(sizeof(int) * 17);

    char *firstStr = (char *)malloc(sizeof(char) * 16);
    fread(firstStr, sizeof(char), 16, in);
    ...
    int constsSize = 0;
    fread(&constsSize, sizeof(int), 1, in);
    ...
    int addrStartProg = 0;
    fread(&addrStartProg, sizeof(int), 1, in);


    int addrStartStack = 0;
    fread(&addrStartStack, sizeof(int), 1, in);


    int idProc = 0;
    fread(&idProc, sizeof(int), 1, in);

    fseek(in , 512 , SEEK_SET);



    fread(mem, sizeof(int), codeSize, in);

    //here can be read the segments: consts, data

    addrStartStack = 1048575;

    regs[15] = addrStartProg;
